...
<template>
  <n-layout-header bordered class="the-header" style="display: flex; align-items: center;">
    ...
    <n-grid cols="24" item-responsive>
      ...
      <!-- <n-grid-item span="2"  v-if="isLoggedIn">
      ...
      <n-grid-item class="login-button" span="2">
        <n-button v-if="!isLoggedIn" type="primary" @click="router.push('/login')"> 登录 </n-button>
        ...
        >
          <template #trigger>
            <n-button type="primary"> 退出登录 </n-button>
          </template>
          是否退出登录
        </n-popconfirm>
